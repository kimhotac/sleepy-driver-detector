"""
적외선 CNN(MLP) 기반 눈 상태 감지 모델 - 기존 InfraredMLPEyeCloseDetector와 100% 동일

PyTorch 기반 CNN 모델을 사용하여 눈의 개폐 상태를 판단합니다.
기존 로직을 절대 변경하지 않고 새로운 인터페이스에 맞는 wrapper만 추가했습니다.
"""
...
class EyeCNN(nn.Module):
    """기존 InfraredMLPEyeCloseDetector와 정확히 동일한 CNN 아키텍처"""
    def __init__(self):
        super().__init__()
...
class InfraredMLPEyeModel(EyeStateDetector):
    """기존 InfraredMLPEyeCloseDetector를 100% 그대로 복사한 구현"""
    ...
    def __init__(self):
        ...
        model_path = os.path.join(base_dir, "infrared_model.pth")
    ...
    def predict(self, eye_img):
        """기존 InfraredMLPEyeCloseDetector.predict와 100% 동일한 로직"""
        if eye_img.size == 0:
            return None
